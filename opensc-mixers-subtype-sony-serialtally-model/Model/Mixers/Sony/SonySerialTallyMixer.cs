...
namespace OpenSC.Model.Mixers.SonySerialTally
{
    ...
    public class SonySerialTallyMixer : Mixer
    {
        ...
        public void Connect()
        {
            ...
            {
                string errorMessage = string.Format("Couldn't connect to a Sony Serial Tally mixer/switcher (ID: {0}) with IP {1}. Exception message: [{2}]",
                    ID, IpAddress, ex.Message);
                LogDispatcher.E(LOG_TAG, errorMessage);
            }
            catch(AlreadyConnectedException ex)
            {
                string errorMessage = string.Format("Tried to connect to a Sony Serial Tally mixer/switcher (ID: {0}) with IP {1}, but was already connected. Exception message: [{2}]",
                    ID, IpAddress, ex.Message);
        ...
        public void Disconnect()
        {
            ...
            {
                string errorMessage = string.Format("Tried to disconnect from a Sony Serial Tally mixer/switcher (ID: {0}) with IP {1}, but wasn't connected. Exception message: [{2}]",
                    ID, IpAddress, ex.Message);
                LogDispatcher.W(LOG_TAG, errorMessage);
            }
        }
        ...
        public event PropertyChangedTwoValuesDelegate<SonySerialTallyMixer, string> IpAddressChanged;
        ...
        public event PropertyChangedTwoValuesDelegate<SonySerialTallyMixer, bool> ConnectionStateChanged;
            ...
            set
            {
                AfterChangePropertyDelegate<bool> afterChangeDelegate = (ov, nv) =>
                {
                    if (nv)
                    {
                        ...
                        string logMessage = string.Format("Connected to a Sony Serial Tally mixer/switcher (ID: {0}) with IP {1}.", ID, IpAddress);
                    ...
                    else
                    {
                        ...
                        State = MixerState.Warning;
                        StateString = "disconnected";
                        string logMessage = string.Format("Disconnected from a Sony Serial Tally mixer/switcher (ID: {0}) with IP {1}.", ID, IpAddress);
                        LogDispatcher.I(LOG_TAG, logMessage);
        ...
        public event PropertyChangedTwoValuesDelegate<SonySerialTallyMixer, bool> AutoReconnectChanged;
